(function () {
	...
	function process (template) {
		...
		function make (i, braces) {
			pattern = RegExp('(.*?)' + braces.join(' *((?:([=&!#^>/]) *)?([\\S\\s]*?) *?(=)?) *'), 'ig');
			pattern.lastIndex = i;
		}

		function value (_, e) { r += "uv"[~~e] + "(" + _ + ");" }
		function plain (_) { if (_) value("'" + _.replace(/'/g, "\\'") + "'") }
		...
		for (make(0, ['{{', '}}']); match = pattern.exec(template); i = pattern.lastIndex) {
			plain(match[1]);
